...
class Leads extends AdminController
{
    ...
    private function _get_lead_data($id = '')
    {
        ...
        $data['statuses'] = $this->leads_model->get_status();
        $data['sources']  = $this->leads_model->get_source();
        $data['types']  = $this->leads_model->get_lead_type();
    ...
    {
        if (!is_admin()) {
            access_denied('Leads Types');
        }
        $data['sources'] = $this->leads_model->get_lead_type();
        $data['title']   = 'Leads types';
        $this->load->view('admin/leads/manage_types', $data);
    }
    ...
    public function type()
    {
        ...
        if ($this->input->post()) {
            ...
            if (!$this->input->post('id')) {
                ...
                $id = $this->leads_model->add_lead_type($data);
                ...
                $id = $data['id'];
                unset($data['id']);
                $success = $this->leads_model->update_lead_type($data, $id);
                if ($success) {
    ...
    {
        if (!is_admin()) {
            access_denied('Delete Lead Type');
        }
        if (!$id) {
            redirect(admin_url('leads/types'));
        }
        $response = $this->leads_model->delete_type($id);
        if (is_array($response) && isset($response['referenced'])) {

...
class Leads_model extends App_Model
{
    ...
    public function get($id = '', $where = [])
    {
        ...
        $this->db->join(db_prefix() . 'leads_type', db_prefix() . 'leads_type.id=' . db_prefix() . 'leads.lead_type', 'left');
     ...
     */
    public function get_lead_type($id = false)
    {
        if (is_numeric($id)) {
            $this->db->where('id', $id);
     ...
     */
    public function add_lead_type($data)
    {
        $this->db->insert(db_prefix() . 'leads_type', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            log_activity('New Leads Type Added [TypeID: ' . $insert_id . ', Name: ' . $data['name'] . ']');
        }
     ...
     * @return boolean
     */
    public function update_lead_type($data, $id)
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'leads_type', $data);
        if ($this->db->affected_rows() > 0) {
            log_activity('Leads Type Updated [TypeID: ' . $id . ', Name: ' . $data['name'] . ']');
     ...
     /**
     * Delete lead type from database
     * @param  mixed $id source id
     * @return mixed
     */
    public function delete_type($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'leads_type');
        if ($this->db->affected_rows() > 0) {
            if (get_option('leads_default_source') == $id) {
                update_option('leads_default_source', '');
            }
            log_activity('Leads Type Deleted [SourceID: ' . $id . ']');

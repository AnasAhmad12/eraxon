...
// Items Extra  
	...
	if (!$CI->db->table_exists(db_prefix() . 'assets_available_inventory')) {
		$CI->db->query('CREATE TABLE `' . db_prefix() . 'assets_available_inventory` (
			`id` INT(11) NOT NULL AUTO_INCREMENT,
			`item_id` INT(11) NOT NULL,
	...
	if (!$CI->db->table_exists(db_prefix() . 'assets_allocation')) {
		$CI->db->query('CREATE TABLE `' . db_prefix() . 'assets_allocation` (
			`id` INT(11) NOT NULL AUTO_INCREMENT,
			`allocation_master_id` INT(11) NOT NULL,
			`item_id` INT(11) NOT NULL,
			`serial_number`  VARCHAR(255) NOT NULL,
			`qty` INT NOT NULL DEFAULT "1",
 			PRIMARY KEY (`id`)
	...
	if (!$CI->db->table_exists(db_prefix() . 'assets_allocation_master')) {
		$CI->db->query('CREATE TABLE `' . db_prefix() . 'assets_allocation_master` (
			`id` INT(11) NOT NULL AUTO_INCREMENT,
	...
	if (!$CI->db->table_exists(db_prefix() . 'assets_lost_inventory')) {
		$CI->db->query('CREATE TABLE `' . db_prefix() . 'assets_lost_inventory` (
			`id` INT(11) NOT NULL AUTO_INCREMENT,

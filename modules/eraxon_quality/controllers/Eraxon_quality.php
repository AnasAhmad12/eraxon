...
class Eraxon_quality extends AdminController
{
    ...
    public function get_campaign_sheet($id){
        ...
        $staff_role=$this->roles_model->get_staff_role($staff_data->role);
        if($staff_role){
            $camp_col= $this->eraxon_quality_model->get_campaign_columns($id,$staff_role);
            $leads=$this->eraxon_quality_model->get_leads($id,$staff_role);
        }
        else{
            $camp_col= $this->eraxon_quality_model->get_campaign_columns($id,$staff_role);
            $leads=$this->eraxon_quality_model->get_leads($id,$staff_role);

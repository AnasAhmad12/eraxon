...
$(function() {
    ...
    $(".quantity").change(function(event) {
        ...
        setTimeout(function() {
            ...
            update_cart = $.post(site_url+'products/kiosk/add_cart', {quantity: quantity, product_id: product_id, product_variation_id: product_variation_id}, function(data, textStatus, xhr) {});
            rate = that.data('rate');
            ...
            $(".ptotal").text(format_money(total))
            $(".psubtotal").text(format_money(subtotal));
        }, 500);
    });
    $(".remove_cart").on('click', function(event) {
        ...
        var button = $(this);
        $.post(site_url+'products/kiosk/remove_cart', {product_id: product_id, product_variation_id: product_variation_id}, function(data, textStatus, xhr) {
            removing = false;

            data = $.parseJSON(data);
            if (data.status == false) {
                window.location.href = site_url+'products/kiosk';
                return false;
            }
...
function init_currency() {
    $.get(site_url + 'products/kiosk/get_currency/'+base_currency)

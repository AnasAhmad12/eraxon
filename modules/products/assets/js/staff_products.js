...
$(function() {
    ...
    $(document).on('click', '.add_cart', function(event) {
        ...
        $.post(site_url+'products/kiosk/add_cart', {quantity: quantity, product_id: product_id, product_variation_id: product_variation_id}, function(data, textStatus, xhr) {
            cart_items = $.parseJSON(data);
            $('#checkout').html(cart_items.length);
...
function filter_data(post_data = {}) {
    ...
    $(".image_loader").show();
    $.ajax({
        url: site_url+'products/kiosk/filter',
        type: 'POST',
        dataType: 'json',
        data : post_data,
...
function render_product_data(data) {
            ...
            <div class="thumbnail shadow">
                <div>
                    ...
                    <div class="d-flex justify-content-center" style="display:inline-flex">
                        <div class="text-center badge badge-pill badge-primary">${val.p_category_name}</div>
                    </div>
                    <div class="rates products-pricing">
                        <h4>${val.base_currency_name} <span style="font-weight: bold;" class="bold product-price">${product_rate}</span> ${recurring_type} ${total_taxes}</h4>
    ...
    $.each( cart_items, function( key, value ) {
        // console.log( key + ": " + Object.keys(value).length);//JSON.stringify(value) );
           if(Object.keys(value).length > 0)
           {
               cart_counter++;
           }
       });
       $('#checkout').html(cart_counter);
    $("#filter_html").hide();
    $(".image_loader").hide();

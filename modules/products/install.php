...
if (!$CI->db->field_exists('product_code', db_prefix() . 'product_master')) {
    $CI->db->query('ALTER TABLE `' . db_prefix() . 'product_master`
        ADD `product_code` VARCHAR(150) NULL AFTER `product_name`');
}
...
//Anas Ahmad
if (!$CI->db->field_exists('trans_id', db_prefix() . 'order_master')) {
    $CI->db->query('ALTER TABLE `' . db_prefix() . 'order_master`
        ADD `trans_id` INT(11) NULL AFTER `status`');
}
//Anas Ahmad
if (!$CI->db->field_exists('product_code', db_prefix() . 'product_variations')) {
    $CI->db->query('ALTER TABLE `' . db_prefix() . 'product_variations`
        ADD `product_code` INT(11) NULL AFTER `variation_value_id`');
}
//Anas Ahmad
if (!$CI->db->field_exists('order_type', db_prefix() . 'order_master')) {
    $CI->db->query('ALTER TABLE `' . db_prefix() . 'order_master`
        ADD `order_type` VARCHAR(50) NULL AFTER `status`');
}


//Shoaib Tariq 

if (!$CI->db->table_exists(db_prefix().'product_purchases')) {
    $CI->db->query('CREATE TABLE `'.db_prefix().'product_purchases` (
        `id` INT NOT NULL AUTO_INCREMENT,
...
if (!$CI->db->table_exists(db_prefix().'product_purchase_items')) {
    $CI->db->query('CREATE TABLE `'.db_prefix().'product_purchase_items` (
        `id` INT NOT NULL AUTO_INCREMENT,
        ...
        `product_variation_value_id` INT(11) DEFAULT NULL,
        `session_id` VARCHAR(20) NOT NULL,
        `net_unit_cost` DECIMAL(25,2) NOT NULL,

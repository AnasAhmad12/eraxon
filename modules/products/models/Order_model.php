...
class Order_model extends CI_Model
{
    ...
    public function add_pos_order($order_master,$o)
    {
        $product_items       = $o;
        $this->db->insert(db_prefix() . 'order_master', $order_master);
    ...
    public function add_invoice_staff_order($post)
    {
        if (empty($post)) {
            return ['status' => false, 'message' => "Post data cannot be empty`"];
        }
       ...
       // unset($post['shipping_country']);
        ...
        if (!empty($post['newitems'])) {
           ...
           // $id               = $this->invoices_model->add($post);
            ...
            if (1) {
                $result[]                 = true;
              //$res                      = $this->invoices_model->get($id);
                $order_data['status']     = 1;//$res->status;
              //  $order_data['invoice_id'] = $id;
                 $id = $this->order_model->add_order($order_data);
        ...
        if (count($invoice_insert_items) + $count == count($result)) {
            if (1 == count($result)) {
               // return ['status' => true, 'single_invoice' => true,'invoice_id' => $id, 'invoice_hash' => $res->hash];
                return ['status' => true,'invoice_id' => $id];//, 'invoice_hash' => $res->hash];
            }
            return ['status' => true, 'single_invoice' => false];
        }
        return ['status' => false, 'message' => _l('order_fail')];
    ...
    public function order_status($order_id, $status)
    {
        ...
        $this->db->update(db_prefix() . 'order_master', ['status' => $status]);

    }
    public function delete_order($order_id)
    {
        $this->db->where('id', $order_id);
        $this->db->delete(db_prefix() . 'order_master');
  ...
  public function  update_order($order_master_id,$item,$subtotal,$trans_id){

    $this->db->where('id',$order_master_id);
    $this->db->update(db_prefix().'order_master',["subtotal"=>$subtotal,"total"=>$subtotal]);
    
    foreach($item as $i){
        $this->db->where('order_id',$i['order_id']);
        $this->db->where('product_id',$i['product_id']);
        $this->db->where('product_variation_id',$i['product_variation_id']);
        $this->db->update(db_prefix().'order_items',["qty"=>$i['qty']]);
  ...
  public function today_order(){
    ...
    $this->db->join(db_prefix().'staff', db_prefix().'staff.staffid = order_master.clientid', 'LEFT');
 
    $orders=$this->db->get(db_prefix().'order_master')->result();


    // $options="";
    // $html="";
    // foreach( $orders as $aRow){
    ...
    // $html .='<td>'.$aRow->order_date.'</td>';
    // $html .='<td>'."PKR".' '.$aRow->total.'</td>';
   ...
   return $orders;
    ...
    public function update_order_transaction($order_id, $trans)
    {
        $this->db->where('id', $order_id);
        $this->db->update(db_prefix() . 'order_master', ['trans_id' => $trans]);

    }


    public function get_kiosk_last_three_order_by_user_id($staffid){


        $this->db->select(['id','total','order_date']);

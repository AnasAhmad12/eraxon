...
class Products_model extends App_Model
{
                    ...
                    if ($variation_row) {
                        ...
                        if ($variation_value_row) {
                            $product_variation_data = [
                                ...
                                'product_code' => $variations['product_code'][$variation_index],
    ...
    public function get_by_name_product($name = false)
    {

        if (strlen($name)==13) 
        {
           /* $this->db->join(db_prefix() .'product_master', db_prefix() . 'product_master.id=' . db_prefix() . 'product_variations.product_id', 'LEFT');

            $this->db->like(db_prefix() .'product_variations.product_code', $name);
            $product = $this->db->get(db_prefix() . 'product_master')->result();*/


             $this->db->where('product_code',$name);
             $product = $this->db->get(db_prefix() . 'product_master');
            ...
            $this->db->where('product_code',$name);
            $variation = $this->db->get(db_prefix() . 'product_variations');

            if($variation->num_rows() > 0 )
            {
                $variation = $variation->result();
                $this->db->where('id',$variation[0]->product_id);
                $product = $this->db->get(db_prefix() . 'product_master')->result();
                ...
                if ($product_row->is_variation) {
                    ...
                    $this->db->join('variations', db_prefix() . 'variations.id=' . db_prefix() . 'product_variations.variation_id', 'LEFT');
                    $this->db->join('variation_values', db_prefix() . 'variation_values.id=' . db_prefix() . 'product_variations.variation_value_id', 'LEFT');
                    $this->db->where('variation_value_id', $variation[0]->variation_value_id);
                    $this->db->where('product_id', $product_row->id);
                    $this->db->order_by('variation_id');
                    $product_row->variations = $this->db->get(db_prefix() . 'product_variations')->result();
                }
